...
class Paydo extends PaymentModule
{
    private $_html = '';
    ...
    public function __construct()
    {
        ...
        $this->bootstrap              = true;
        $this->displayName            = 'PayDo';
        $this->description            = 'Make payments via PayDo';
    ...
    public function processConfiguration()
    {
        if (Tools::isSubmit('pc_form')) {
            ...
            Configuration::updateValue('PAYDO_ENABLE', $enablePayments);
            Configuration::updateValue('PAYDO_NAME', $displayName);
            Configuration::updateValue('DESCRIPTION', $description);
            Configuration::updateValue('PAYDO_PUBLIC_KEY', $publicKey);
            Configuration::updateValue('PAYDO_SECRET_KEY', $secretKey);
            ...
            Configuration::updateValue('PAYDO_LANGUAGE', $language);
    ...
    {
        $enablePayments = Configuration::get('PAYDO_ENABLE');
        $displayName = Configuration::get('PAYDO_NAME');
        $description = Configuration::get('DESCRIPTION');
        $publicKey = Configuration::get('PAYDO_PUBLIC_KEY');
        $secretKey = Configuration::get('PAYDO_SECRET_KEY');
    ...
    {
        $requestUrl = 'https://paydo.com/v1/instrument-settings/payment-methods/available-for-user';
    ...
    public function hookPaymentOptions()
    {
        ...
        if (!(Configuration::get('PAYDO_ENABLE')  === "1")) {
            return;
        }
         ...
         */
        $paymentForm = $this->fetch('module:paydo/views/templates/hook/payment_options.tpl');
        ...
        $newOption->setModuleName($this->displayName)
          ->setCallToActionText(Configuration::get('PAYDO_NAME'))
    ...
    {
        if (Configuration::get('PS_OS_PAYDO_PENDING_STATE') < 1) {
            ...
            if ($order_state->add()) {
                Configuration::updateValue('PS_OS_PAYDO_PENDING_STATE', $order_state->id);
